...
class EzMCP:
        ...
        async def call_tool(
            ...
            for param_name, param_info in tool_info["params"].items():
                if param_name in arguments:
                    ...
                    raise ValueError(f"Missing required argument '{param_name}' for tool '{name}'")
                else:
    ...
    def _create_tool_schema(self, name: str, description: str, params: Dict[str, ParamInfo]) -> Tool:
        ...
        for param_name, param_info in params.items():
            ...
            if param_info.type in (str, Annotated[str, ...]):
                ...
                type_name = "boolean"
            elif param_info.type in (list, List, Annotated[list, ...], Annotated[List, ...]):
                type_name = "array"
            elif param_info.type in (dict, Dict, Annotated[dict, ...], Annotated[Dict, ...]):
                type_name = "object"
            else:
